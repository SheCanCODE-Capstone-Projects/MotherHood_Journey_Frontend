...
import { Home, FileText, ClipboardList, MessageSquare, Lightbulb, Send, Paperclip, Smile, Phone, Video, MoreVertical, AlertCircle, FileIcon, MapPin, Menu, X, User } from "lucide-react";
...
function ChatWindow({ activeChat }: { activeChat: ChatMember }) {
...
function ChatHeader({ activeChat }: { activeChat: ChatMember }) {
  ...
  return (
    <div className="px-4 md:px-6 py-4 border-b border-gray-100 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={() => router.push(`/mothers/${activeChat.motherId}`)}
          className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors"
        >
          <User size={14} className="text-teal-600" />
          <span className="text-[10px] font-semibold text-teal-600">View Profile</span>
        </button>
